...
function rcxDict(loadNames, rcxConfig) {
...
rcxDict.prototype = {
	...
	loadNames: function() {
		...
		let dataPath = rcxConfig.namesDatPath || "/js/ja/data/";
		this.nameDict = this.fileRead(dataPath + 'names.dat');
		this.nameIndex = this.fileRead(dataPath + 'names.idx');
	},
	...
	loadDictionary: function() {
		...
		let dataPath = this.rcxConfig.namesDatPath || "/js/ja/data/";
		this.wordDict = this.fileRead(dataPath + 'dict.dat');
		this.wordIndex = this.fileRead(dataPath + 'dict.idx');
		this.kanjiData = this.fileRead(dataPath + 'kanji.dat', 'UTF-8');
		this.radData = this.fileReadArray(dataPath + 'radicals.dat', 'UTF-8'); 
...
*/
	...
	loadDIF: function() {
		...
		let dataPath = this.rcxConfig.namesDatPath || "/js/ja/data/";
		var buffer = this.fileReadArray(dataPath + "deinflect.dat", 'UTF-8');
		var prevLen = -1;
		var g, o;
					...
					for (k = 0; k < g.length; ++k) {
						...
						if ((type & rule.type) && (end == rule.from)) {
							...
							if (r[i].reason.length) {
								o.reason = this.difReasons[parseInt(rule.reason)] + ' &lt; ' + r[i].reason;
							} else {
								// console.log(rule.reason, this.difReasons[parseInt(rule.reason)], this.difReasons);
								o.reason = this.difReasons[parseInt(rule.reason)];
							}
							o.type = rule.type >> 8;
							o.word = newWord;
	...
	makeHtml: function(entry) {
		...
		if (entry.kanji) {
			...
			box = '<table class="k-abox-tb"><tr>' +
				...
				'</tr></table>';
			if (this.rcxConfig.kanjicomponents) {
			...
			kanjiinfo = this.rcxConfig.kanjiinfo;
